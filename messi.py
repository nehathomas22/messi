...
try:
  ...
  from moviepy import ImageClip,cocatenate_videoclip,audioFileclip
  import yt_dlp
...
def download_youtube_audio(url)
 """Downloads only audio from youTube using reliable browser impersonation."""
  audio_opts =[
    ...
    'http_headers':{
        'user=Agent':'Mozila/5.0 (windows Nt 10.0;win64;x64) ApplewebkIT/537. (KHTML,like Gecko) chrome/1
    'Accept':'*/*',
...
def handle_youtube_download(url):
   """callback function to ensure session state presists after button click"""
  try:
    #clear previous errors
    if'yt_errors' in st.session_state:
  del st.session_state['yt_error']
